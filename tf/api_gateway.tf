...
resource "aws_api_gateway_resource" "referral" {
  rest_api_id = aws_api_gateway_rest_api.course_api.id
  parent_id   = aws_api_gateway_rest_api.course_api.root_resource_id
  path_part   = "referral"
}
...
# Referral endpoint methods and integrations
resource "aws_api_gateway_method" "referral_post" {
  rest_api_id   = aws_api_gateway_rest_api.course_api.id
  resource_id   = aws_api_gateway_resource.referral.id
  http_method   = "POST"
  authorization = "NONE"
}

resource "aws_api_gateway_method" "referral_options" {
  rest_api_id   = aws_api_gateway_rest_api.course_api.id
  resource_id   = aws_api_gateway_resource.referral.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "referral_options" {
  rest_api_id = aws_api_gateway_rest_api.course_api.id
  resource_id = aws_api_gateway_resource.referral.id
  http_method = aws_api_gateway_method.referral_options.http_method
  type        = "MOCK"
...
resource "aws_api_gateway_method_response" "referral_options" {
  rest_api_id = aws_api_gateway_rest_api.course_api.id
  resource_id = aws_api_gateway_resource.referral.id
  http_method = aws_api_gateway_method.referral_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "referral_options" {
  rest_api_id = aws_api_gateway_rest_api.course_api.id
  resource_id = aws_api_gateway_resource.referral.id
  http_method = aws_api_gateway_method.referral_options.http_method
  status_code = aws_api_gateway_method_response.referral_options.status_code
...
resource "aws_api_gateway_integration" "referral_integration" {
  rest_api_id             = aws_api_gateway_rest_api.course_api.id
  resource_id             = aws_api_gateway_resource.referral.id
  http_method             = aws_api_gateway_method.referral_post.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.referral_handler.invoke_arn
}
...
resource "aws_api_gateway_deployment" "course_api" {
  depends_on = [
    ...
    aws_api_gateway_integration.referral_integration,
    aws_api_gateway_integration.referral_options
  ]
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_integration.referral_integration.id,
      ...
      aws_lambda_function.referral_handler.source_code_hash
    ]))
  }
...
resource "aws_lambda_permission" "api_gateway_referral" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.referral_handler.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.course_api.execution_arn}/*/*"
}
