...
test_event_code = TEST_EVENT_CODE if not is_production else None
...
# Prepare test CompleteRegistration event for WEBSITE pixel
event_data = {
    'data': [{
        ...
        'event_source_url': f'https://anyone-can-build.com/register?test={unique_id}',
        'event_id': f'registration_{registration_id}',
        'custom_data': {
...
print(f"Sending CompleteRegistration {event_type} event to Pixel {pixel_id}...")
print(f"Using Email: {random_email}, Phone: {random_phone}, Registration ID: {registration_id}")
...
if 'events_received' in result:
    print(f"✅ Success! {result['events_received']} CompleteRegistration event(s) received")
    if is_production:
        ...
        print("5. You should see a 'CompleteRegistration' event in the test events")
else:
    print("❌ Error:", result)